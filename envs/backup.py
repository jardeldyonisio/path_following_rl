...
class MultiAgentPathFollowingEnv(gym.Env):
    def __init__(self, num_agents=20, buffer_size=1000):
        ...
        self.replay_buffer = []  # Replay buffer para armazenar transições
        
        # Action space: [steering, speed] for each agent
        ...
        # Observation space: [x, y, distance_to_path, angle_to_path] per agent
    ...
    def reset(self):
        self.positions = np.zeros((self.num_agents, 2))  # Posições iniciais
        self.orientations = np.zeros(self.num_agents)  # Orientações iniciais
        self.path = self.generate_path()
        self.time = 0
        self.active_agents = np.ones(self.num_agents, dtype=bool)  # Agentes ativos
    ...
    def step(self, actions):
        ...
        # Marcar agentes como inativos se atingirem a distância de 5.0
        self.active_agents = self.active_agents & (distances <= 5.0)

        # Armazenar a transição no replay buffer
        next_state = self.get_state()
        for i in range(self.num_agents):
            self.replay_buffer.append((self.get_state(), actions[i], rewards[i], next_state[i]))
        
        # Garantir que o replay buffer não ultrapasse o tamanho máximo
        if len(self.replay_buffer) > self.buffer_size:
            self.replay_buffer.pop(0)

        self.time += 1
        done = self.time > 500 or np.sum(self.active_agents) == 0  # Acaba o episódio
...
while True:
    ...
    for _ in range(500):
        ...
        env.render()
        if done:
            print(f"Episode ended: Time={env.time}")
            break
